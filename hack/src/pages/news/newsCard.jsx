...
export default function NewsCard({ news }) {
  return (
    ...
    {news && news.length > 0 ? news.map(n => (
      <div key={n.title} className="flex-none rounded-lg p-4 w-80 bg-white shadow-lg hover:shadow-xl transition-shadow duration-300" style={{ height: "400px", overflow: "hidden" }}>
        <Card sx={{ maxWidth: 345, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <CardMedia
            component="img"
            alt="Image not available"
            height="140"
            image={n.urlToImage || 'https://via.placeholder.com/140'}
          />
          <CardContent style={{ flexGrow: 1, overflow: 'hidden' }}>
            ...
            <Typography variant="body2" color="text.secondary" style={{ fontSize: '0.875rem', lineHeight: '1.4em', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
